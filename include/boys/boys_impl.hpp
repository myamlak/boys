...
// The single-precision piece at the piece's effective degree, summed by the
// named scheme: the same pair of stored tables the full-degree helper above
// reads, at the degree the rung's criterion certifies. The two tables are
// parallel, so a scheme picks one and nothing else about the fit changes.
template <EvalScheme kScheme = kDefaultEvalScheme, typename DegreesArray>
float ChebyshevValueF32WithDegrees(int order, float x, const DegreesArray& degrees) noexcept {
    ...
    const float* m = detail::f32::kMonoCoeffs.data() + piece.offset;
    const float t = 2.0f * (x - piece.a) / (piece.b - piece.a) - 1.0f;
    return FitSum<kScheme, backend::ScalarFp32>(c, m, degrees[static_cast<std::size_t>(index)], t);
}
...
inline float RegionBSeedF32WithDegrees(float x, int degree) noexcept {
    ...
    return FitSum<kScheme, backend::ScalarFp32>(
        detail::f32::kBcoeffs.data(), detail::f32::kMonoBcoeffs.data(), degree, t);
}
