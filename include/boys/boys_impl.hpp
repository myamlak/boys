...
float BoysSingleF32Impl(int n, float x) noexcept {
    ...
    } else
    {
        ...
        if (x < x1)
        {
            ...
            // The same spelled two-rounding step as the reference rung's body
            // above, and for the same reason: a bare product and difference
            // would make this rung's value a property of the calling unit.
            for (int l = 0; l < n; ++l)
            {
                f = backend::ScalarFp32::MulSub(static_cast<float>(l) + 0.5f, f, expx) / x;
            }
