...
Each is a static assertion with the reason, and each names the table it would need — a narrow
rational pair, a narrow float table. Neither falls back: the two partitions are different fits of the
same function over the same interval, so a substitution would return the shipped numbers under the
narrow partition's name. Both are unbuilt tables rather than unavailable options, and each is named
